...
// 16년 5월 29일 변경사항 - 최승환 :
/* 기존에 스키마 생성시 문장 중간에 있던 PRIMARY KEY를 뒤쪽으로 이동
 * 데이터베이스에 맞지 않던 varchar2 변수를 varchar로 모두 변경함.
 */
...
public class XmlToDBSchema {
	...
	private String SettingSql( String sql, String logTable, String logTableSeq){
		if( !logTableSeq.equals("") ){
			...
			sqlSet += tableName + "( wts Timestamp NOT NULL, siteID varchar(8) NOT NULL, ";
			
			sqlSet += logTable.equals("logTable") ? "" : ""; 
			try{
	...
	private String SeqCreate( String sql ){
		...
		sqlSet += tableName + " AS INTEGER start with 1 INCREMENT BY 1 CACHE 20 MINVALUE 1 MAXVALUE 9999 CYCLE ORDER ";
		return sqlSet;
	}
...
//				실험을 위해 외래키 삭제+ ", CONSTRAINT FK_" + sql + " FOREIGN KEY (" + sql +") " + "REFERENCES " + tableName + " (oid), ";
	...
	private String primarykeySetting( String logTable, int bool ){
		String returndata = " oid varchar(11)";
		String primaryKey = logTable.equals("logTable") ? "" : ", PRIMARY KEY (wts)";
	...
	public void createDBSchma(){
		...
		try {
			...
			//insertIntoDB(objectDatas,"logTableSeq","seq"); //시퀀스 생성부분. 오류가 있어 주석처리 함.
